...
public class Gaussian {
    ...
    public static void main(String[] args) {
    		double mu    = 0;
        double sigma = Math.sqrt(0.2);
        double z     = 3 * sigma;
        double step = 6 * sigma / 10;
        for (double i = -3 * sigma + step; i <= 3 * sigma; i += step) {
        		System.out.println(pdf(i, mu, sigma) + "--" + cdf(i, mu, sigma));
        }
        
        System.out.println(cdf(z, mu, sigma));
        System.out.println(pdf(z, mu, sigma));

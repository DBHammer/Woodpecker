...
public class Comparer
{
     ...
     */
    public static <T> boolean verify(T input1, T input2, String operator, boolean isSetType)
    {
        ...
        else if (dataType1.matches(".*ResultImpl") && dataType2.matches(".*ResultImpl"))
        {
            ...
            return equals((Result) input1, (Result) input2, isSetType);
        }
     ...
     */
    public static <T> boolean verify(T[] input1, T[] input2, String operator)
    {
        if (input1.length != input2.length)
            return false;
        for (int i = 0; i < input1.length; i++)
        {
            boolean result = verify(input1[i], input2[i], operator, false);
            if (result == false)
                return false;
        }
        return true;
     ...
     * @return 比较结果。input1与input2中元素按序相等则返回true，否则返回false
     */
    public static <T> boolean verify(List<T> input1, List<T> input2, String operator)
    {
        if (input1.size() != input2.size())
            return false;
        for (int i = 0; i < input1.size(); i++)
        {
            boolean result = verify(input1.get(i), input2.get(i), operator, false);
            if (result == false)
                return false;
        }
        return true;
     ...
     * @return
     */
    private static boolean equals(Result result1, Result result2, boolean isSetType)
    {
        // 大小不一致则不相等
        if (result1.getRowCount() != result2.getRowCount() || result1.getColumnNames().length != result2.getColumnNames().length)
            return false;
        // Compare strictly
